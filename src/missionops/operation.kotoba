(ns missionops.operation
  "OperationActor -- one mission-operations operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (MissionOps-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Mission Operations Governor
  ...
  One graph run = one mission-operations operation (intake -> advise
  ...
  decision to a human operator (mission-operations coordinator). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:deployment/dispatch`/`:deployment/report` ALWAYS reach
  this node when the governor is clean -- see `missionops.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [missionops.missionopsllm :as missionopsllm]
            [missionops.governor :as governor]
...
(defn build
  ...
  `missionops.store/Store`).
  opts:
    :advisor      -- a `missionops.missionopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (missionopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; MissionOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (missionopsllm/-advise advisor store request)]
            {:proposal p :audit [(missionopsllm/trace request p)]})))

      ;; Mission Operations Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
